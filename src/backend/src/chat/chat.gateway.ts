...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
 ...
 async handleConnection(socket: Socket) {
  ...
  async createRoom(@MessageBody() name: string, roomType: RoomType, owner: User, password?: string) {
    if (roomType === 'PROTECTED' && !password)
      return 'Error';
    this.roomService.createRoom({ name, password, roomType, users: {create: {user: {connect: owner}, role: 'OWNER'}} });
    // needs emit once i can find how to identify user by connection to server
  }

  @SubscribeMessage('createMessage')
  async createMessage(@MessageBody() content: string, room: Room, user: User) {
