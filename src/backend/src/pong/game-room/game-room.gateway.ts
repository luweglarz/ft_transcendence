...
export class GameRoomGateway {
  constructor(
    ...
    private gameCoreGateway: GameGateway,
  ) {}
  ...
  createCustomGame(client: Socket, roomName: string) {
    const newRoom: Room = this.gameRoomService.createGameRoom(client, roomName);
  ...
  joinGameRoom(client: Socket, roomName: string) {
    ...
    this.gameCoreGateway.server
      .to(
        this.rooms[this.rooms.findIndex((element) => element.name === roomName)]
          .name,
      )
      .emit('roomJoined');
    this.logger.log(
